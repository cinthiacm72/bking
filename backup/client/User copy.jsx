...
const User = () => {
  ...
  //console.log("reservationIdToCancel: ", reservationIdToCancel);
  // console.log("Data: ", reservationsList);

  //let selectedRooms = [];
  //let roomId = [];
  // let allDates = [];
  // let idReservationToCancel = [];

  if (reservationsList.length > 0) {
    // selectedRooms = reservationsList.map((item) =>
    //  item.selectedRooms.map((roomNumberId) => roomNumberId)
    // );
    // roomId = reservationsList[0].roomId;
    // allDates = reservationsList[0].allDates;
  }

  //console.log("selectedRooms: ", selectedRooms);

  /*   if (reservationIdToCancel.length > 0) {
    idReservationToCancel = data.filter(
      (item) => item._id === reservationIdToCancel
    )[0].selectedRooms;
  }
 */
  ...
  const handleCancelReserve = async (reservation) => {
    ...
    try {
      const res = fetch(
          ...
          `/reserves/delete/${reservation._id}`,
        {
          ...
          /*  body: JSON.stringify(AllDatesParsed), */
        }
      );

      /* setTimeout(() => {
        setReservationsList(
          reservationsList.filter((item) => item._id !== reservationId)
        );
      }, 1000); */
    } catch (err) {
      console.log(err);
            ...
            <ul className='hotel-list reset-list'>
              {reservationsList.length === 0 ? (
                ...
                reservationsList.map((item) => (
                  <li key={item._id} className='hotel-item' data-rule>
                    ...
                    <div className='hotel-item-info'>
                      ...
                      <h4 className='fs-large margin-top-sm bold'>
                        {item.reservesRooms.title} Room
                      </h4>
                      <p className='fs-small'>
                        Rooms: {item.selectedRooms.length}
                      </p>
                      ...
                      <div className='flex flex-gap-sm '>
                        ...
                        <button onClick={() => handleFiltrar(item)}>
                          Filtrar
                        </button>
                      </div>
                    </div>
